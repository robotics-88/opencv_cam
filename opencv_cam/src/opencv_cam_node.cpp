...
std::string get_gstreamer_pipeline(const std::string &filename) {
    ...
    std::string get_capture_pipeline(const std::string &device, int width, int height, int fps) {
        ...
        if (model.find("Orin") != std::string::npos ||
            ...
            if (format == "MJPG") {
                pipeline
                    << "v4l2src device=" << device << " ! image/jpeg, width=" << width
...
#define CXT_MACRO_MEMBER(n, t, d) CXT_MACRO_CHECK_CMDLINE_PARAMETER(n, t, d)
        ...
        if (camera_info_msg_.distortion_model == "fisheye") {
            K_fisheye_ =
                cv::Mat(3, 3, CV_64F, const_cast<double *>(camera_info_msg_.k.data())).clone();
            D_fisheye_ = cv::Mat(1, camera_info_msg_.d.size(), CV_64F,
                                 const_cast<double *>(camera_info_msg_.d.data()))
                             .clone();
            R_fisheye_ =
                cv::Mat(3, 3, CV_64F, const_cast<double *>(camera_info_msg_.r.data())).clone();
            P_fisheye_ =
                cv::Mat(3, 4, CV_64F, const_cast<double *>(camera_info_msg_.p.data())).clone();
    ...
    void OpencvCamNode::handleSee3CamFrame(cv::Mat & frame, rclcpp::Time stamp) {
        ...
        if (SeparatingRGBIRBuffers(frame, &IRImageCU83, &RGBImageCU83, &RGBBufferSizeCU83,
            ...
            if (!RGBImageCU83.empty()) {
                ...
                image_msg->step =
                    static_cast<sensor_msgs::msg::Image::_step_type>(ResultImage.step);
            ...
            if (!IRImageCU83.empty()) {
                ...
                image_msg->is_bigendian = false;
                image_msg->step =
                    static_cast<sensor_msgs::msg::Image::_step_type>(IRImageCU83.step);
...
#undef SHOW_ADDRESS
        ...
        if (camera_info_msg_.distortion_model == "fisheye") {
            ...
            rectified_msg->step =
                static_cast<sensor_msgs::msg::Image::_step_type>(rotated_image.step);
